        ...
        return (
            <div className={styles.administrator} key={admin.id + '_admins'}>
                ...
                <div className={styles.descriptions}>
                    ...
                    <div>{admin.descriptions || 'без описания'}</div>
                </div>
                    ...
                    {addPattern ? (
                        <div className={styles.addChart}>
                            <select value={selectAddChart} onChange={(event) => setSelectAddChart(+event.target.value)}>
                                {charts.map((chart) => (
                                    <option value={chart.id} key={chart.id + '_forAdd'}>
                                        {chart.id} : {chart.name}
                                    </option>
                                ))}
                            </select>
                    ...
                    {JSON.parse(admin.charts).map((chart_id: number, idx: number) => {
                        ...
                        return (
                            <div className={styles.chartItem} key={chart_id + '_addedCharts'}>
                                ...
                                <span className={styles.chartDel} onClick={() => deleteChartFromAdministrator(admin.id, chart_id, updateOrgScheme)}>
                                    <img src="svg/org/chart_del.svg" />
                                </span>
                            </div>
                        );
                    })}
        ...
        return (
            <div className={styles.administrator} key={admin.id + '_admins'}>
                <div className={styles.administratorHead}>
                    ...
                    {isAdmin && (
                        <span onClick={() => deleteSectionAdministrator(admin.id, updateOrgScheme)}>
