...
export default function Depatment({ departmentItem, users, userById, updateOrgScheme, office_id, charts, indexDep, isAdmin }: { departmentItem: DepartmentI; users: Array<UserFullI>; userById: any; updateOrgScheme: any; office_id: number; charts: any[]; indexDep: number; isAdmin: boolean }) {
    ...
    const creaetDepartmentHandle = () => {
        createSection(inputSectionName, inputDescriptions, office_id, departmentItem.id, inputCkp, +inputLeadership).then((data) => {
            if (data) {
                updateOrgScheme();
                setInputSectionName('');
                setInputDescriptions('');
    ...
    const updateDepartmentHandle = () => {
        updateDepatment(departmentItem!.id, departmentName, departmentLeadership, departmentCode, departmentDescriptions, departmentCkp, () => {
            updateOrgScheme(), setUpdated(false);
        });
    };

    useEffect(() => {
        if (departmentItem!.name !== departmentName || departmentItem!.descriptions !== departmentDescriptions || departmentItem!.ckp !== departmentCkp || departmentItem?.leadership !== departmentLeadership || departmentItem.code !== departmentCode) setUpdated(true);
        ...
        <div className={styles.department}>
                                ...
                                {departmentItem.patterns.map((id) => {
                                    ...
                                    return (
                                        <div>
                                            ...
                                            {isAdmin && (
                                                <span style={{ cursor: 'pointer' }} onClick={() => delDepartmentPattern(departmentItem.id, id)}>
                                                    ❌
                                                </span>
                                            )}
                                        </div>
            ...
            <div className={styles.sectionsList}>
                {addSection && (
                    <div className={styles.addSectionForm}>
                        <div className={styles.addForm}>
                            <h3>Новая секция</h3>
                            <span className={styles.addHelp}>Название секции( число через пробел в начале для сортировки)</span>
                            <input type="text" value={inputSectionName} onChange={(event) => setInputSectionName(event.target.value)} placeholder="1 название секции" />
                ...
                {departmentItem.sections
                    .toSorted((a, b) => (parseInt(a.name.split(' ')[0]) || 999) - (parseInt(b.name.split(' ')[0]) || 999))
                    .map((section, index: number) => (
                        <Section key={section.id + '_sectionItem'} sectionItem={section} {...{ charts, users, userById, updateOrgScheme, office_id, department_id: departmentItem.id, index, isAdmin }} />
                    ))}
            </div>
        </div>
    );
