...
export default function ChartView({
    ...
    const lineTrendToggle = (line: CostumLineI | undefined) => {
        ...
        if (line)
            setColumnsLinesArr(state => state.map(lineState => lineState.columnKey == line.columnKey ? { ...lineState, trend: !lineState.trend } : lineState));
    }
    ...
    const onCalcTrend = () => {
       ...
       // line&&lineTrendToggle(line);
        ...
        setColumnsLinesArr(state => [...state,
        {
            color: colorInput,
    ...
    useEffect(() => {
        if (selectedLine) {
            setColumnsLinesArr(state => state.map(line => {
    ...
    if (currentPattern)
        return (<>
            ...
            <div className={styles.costumLineBlock}>
                {
                    isAdmin&&
                    columnsLinesArr.map && columnsLinesArr.map((costumLine, lineIndex) =>
                        <div key={costumLine.columnKey}
