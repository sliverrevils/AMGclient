    ...
    return (
        <>
            ...
            {createdRows.map((row, rowIndex) => (
                <tr key={rowIndex + "_rowBody"} onContextMenu={(event) => onEditLine(event, rowIndex)} onClick={() => onSelectRow(row)} style={row.some((field) => field.value === "") ? { outline: "2px solid rgba(40, 177, 22, 0.2)" } : {}}>
                    {row.map((item, itemIndex) => (
                        <td key={item.key + "_td"}>{item.value}</td>
                    ))}
                </tr>
            ))}
            <tr>
                <td onClick={onAddNewLine} className={styles.newLineBtn}>
                    добавить
                </td>
            </tr>
        </>
