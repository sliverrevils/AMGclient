...
export default function StatisticControl() {
    ...
    const changeFieldValue = (elIdx, text) =>
        setChartState((state) =>
            state.map((field, idx) => {
                if (idx == elIdx) {
                    field.value = text;
    ...
    const createEditorHtml = () => {
        if (selectedStat) {
            ...
            if (!currentPattern) {
                return (
                    <div className={styles.statEditorBlock}>
                        ...
                        <button onClick={onDeleteRecord} className={styles.deleteBtn}>
                            Удалить запись
                        </button>
                    </div>
                );
            ...
            return (
                <div className={styles.statEditorBlock}>
                    <div className={styles.patternInfo}>{currentPattern.name || "удалённый шаблон⚠️"}</div>
                    ...
                    <button onClick={onUpdateRecord} className={styles.updateBtn}>
                        Обновить запись
                    </button>
                    <div className={styles.infoBlock}>
                        <span className={styles.infoDate}>{new Date(selectedStat.createdAt!).toLocaleString()}</span>
                        <span className={styles.infoUser}>{userByID(selectedStat.created_by)?.name || "пользователь удалён"}</span>

                        <span className={styles.infoUpdate}>
                            <span> обновлена :</span>
                            <span>{selectedStat.updatedAt ? new Date(selectedStat.updatedAt).toLocaleString() : "не обновлялась"}</span>
                            <span>{userByID(selectedStat.updated_by!)?.name || "не обновлялась"}</span>
                        </span>
                    </div>
                    <button onClick={onDeleteRecord} className={styles.deleteBtn}>
                        Удалить запись
    ...
    return (
        <div className={styles.statControlWrapper}>
            {selectedStat && <Modal closeModalFunc={() => setSelectedStat(null)}>{createEditorHtml()}</Modal>}
                        ...
                        <tbody className={styles.tableBody}>
                            {filteredStatsArr.map((stat, statIndex) => (
                                <tr onClick={() => onSelectStat(stat)} key={stat.id}>
                                    ...
                                    <th style={{ fontWeight: 500, fontSize: 10 }}>
                                        {new Date(+stat.dateStart).toLocaleDateString()} - {new Date(+stat.dateEnd).toLocaleDateString()}
                                    </th>
