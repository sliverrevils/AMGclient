...
export default function CreateRaport() {
   // console.log(officesWithLatestPeriodStats)
    ...
    //FUNCS
    //тоглам айтемы
    const targetItemToggle = (target: 'отделы' | 'отделения' | 'секции') =>
      targetsItemArr.includes(target)
        ? setTargetItemArr((state) => state.filter((item) => item !== target))
        : setTargetItemArr((state) => [...state, target]);
    ...
    const OrgItem=({targetItem}:{targetItem:any})=>{
        ...
        switch(statsType){
            case 'main': statsArr=targetItem.mainPattern?[targetItem.mainPattern]:[];
            break;
            case 'additional': statsArr=targetItem.patterns.length?[...targetItem.patterns]:[]; 
            break;
            default:
        ...
        return (
            ...
            <thead>
                ...
                {
                    baseColumns.map((nameColumn, idx)=><th key={Math.random()} onClick={event =>{
                            ...
                            </td>
                            {
                                ...
                                <td key={Math.random()}>

                                    {
                                        ...
                                        <select>
                                        {
                                           stat.dateColumn.raportInfo?.statHeaders?.map(header=><option>{header}</option>)
                                        }
                                    </select>
    ...
    <div className={styles.createRapWrap}>
      ...
      <div className={styles.filterWrap}>
        <div className={styles.targetToggle}>
            <div className={targetsItemArr.includes('отделы')?styles.item_active:styles.item} onClick={()=>targetItemToggle('отделы')}>отделы</div>
            <div className={targetsItemArr.includes('отделения')?styles.item_active:styles.item} onClick={()=>targetItemToggle('отделения')}>отделения</div>
            <div className={targetsItemArr.includes('секции')?styles.item_active:styles.item} onClick={()=>targetItemToggle('секции')}>секции</div>
        </div>
