        ...
        if (periodType == '2 года плюс текущий' || periodType == 'Месячный') {
            ...
            for (let i = secStart, periodEnd = 0, monthStr = ''; i <= secEnd; i += daySec) {
                ...
                if (periodEnd < i + daySec) {
                    ...
                    if (periodType == 'Месячный' && getDayOfWeek(i) == firstDay) { // CALC WEEK
                        ...
                        if (isFullPeriod && !periodsArrTempStr.length) { //add out of range dates
                    ...
                    if (periodType == '2 года плюс текущий') { //CALC MONTH
...

                {
                    periodType == 'Месячный' && <>
                        <select value={firstDay} onChange={event => setFirstDay(Number(event.target.value))}>
                            ...
                            {daysBefore.map((day, idx) => <option key={idx + '_weekday'} value={idx + 1}>{day}</option>)}
                        </select>
                ...
                <>
                    <select value={selectedPattern} onChange={event => setSelectedPattern(+event.target.value)}>
                        <option value={0}>использовать шаблон</option>
                        {
                            ...
                            <option 
                            key={pattern.name + Math.random()} 
                            value={idx + 1} 
                            title={pattern.headers.map((column)=>`[${column.name}]`).join(' ')}>
                                {pattern.name}
                            </option>
                            )
                            )
                    ...
                    <div onClick={createDateTable} className={styles.createTableBtn}>создать таблицу</div>
                </>
            }
            <div onClick={onCancel} className={styles.createTableCancelBtn}>отмена</div>
