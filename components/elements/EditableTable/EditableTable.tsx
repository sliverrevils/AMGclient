...
export default function EditableTable({ saveFunc, descriptionsStr }: { saveFunc: (textJson: string) => void, descriptionsStr: string }) {
    ...
    //rows
    const onAddRow = () => {
        setRows(state => [...state, { id: nanoid(), values: Array(headers.length).fill({}).map(el => ({ id: nanoid(), value: '' })) }])
    }
    ...
    const onChangeRowItem = (event: React.ChangeEvent<HTMLTextAreaElement>, rowIndex: number, id: string) => {
        setRows(state => state.map((row, rowIdx) => {
            if (rowIdx !== rowIndex) return row;
            return {
                ...row, values: row.values.map((item, itemIdx) => {
                    ...
                    <thead>
                        {
                            headers.map((header, headerIndex) =>
                                <th key={header.id}>
                                    ...
                                    {
                                        isAdmin &&
                                        <span onClick={() => onDelHeader(headerIndex, header.id)}>❌</span>
                                    }
                                </th>
                            )
                        }
                    ...
                    <tbody>
                        {
                            rows.map((row, rowIndex) =>
                                <tr key={row.id}>
                                    ...
                                    {
                                        isAdmin &&
                                        <td onClick={() => onDelRow(row.id)} className={`${styles.itemBtn} ${styles.rowDel}`} style={{ fontSize: 10 }}>❌</td>
                                    }
                                </tr>
                            )
                        }
