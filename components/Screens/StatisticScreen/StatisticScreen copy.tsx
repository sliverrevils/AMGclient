   ...
   // useEffect(() => { console.log('📊', { editorColumns, costumLinesArr }) }, [editorColumns]) // SAVE THIS ON BACK
    ...
    const DynamicTableOfStats = ({ modal = false }: { modal: boolean }) => {
       ...
       // useEffect(() => { editorColumns.length && console.log('⚙️📊', editorColumns) }, [editorColumns])// LOG
        ...
        //----------------------------------------------------------------------------------------------------------------- COLUMN EDITOR---
        useEffect(() => {//SELECTED COLUMN EDITOR VALUES
        ...
        //editor column funcs
        const saveColumn = () => {
            setEditorColumns((state) => state.map((column, index: number) => {
        ...
        const columnToLineOnChart = (costumSelectColumn?:number) => { //---------------CREATE AND ADD  LINE TO CHART 📈
            ...
            if(costumSelectColumn){
                ...
                addCostumLine(editorColumns[costumSelectColumn].name, createdTableBody.map(row => Number(row[costumSelectColumn])), editorColumns[costumSelectColumn].color);
                else
            ...
            if (selectedColumn !== null) {
                addCostumLine(columnEditorName, createdTableBody.map(row => Number(row[selectedColumn])), columnEditorColor);
                return
            }
        ...
        const optionsMenu =
            <div className={styles.optionsMenu} ref={optionsMenuRef} style={{ display: showLogicMenu ? 'flex' : 'none' }}>
                ...
                {
                    !!statisticRowsData.length && statisticRowsData[0].map((statRow, statColumnIndex: number) =>
                ...
                {
                    mathArr.map((math, index) => <div key={index + math.value} className={styles.optionItem} onClick={() => {
        ...
        const NameColumnItem = (column: EditorRowI, index: number) => { //COLUMNS -  NAMES ROW
            const [nameInput, setNameInput] = useState(column.name);
            const saveNewName = () => setEditorColumns(state => {
                const temp = [...state];
            ...
            return <th
                ...
                key={Math.random()}
                onClick={event => { // --------------------------------------------------------------------- OPEN COLUMN RB MENU------- !!!
                    event.preventDefault();
        ...
        const InitialValueRowItem = (column: EditorRowI, index: number) => { //INIT VALUE ROW
            let init = column.initValue == null ? '' : column.initValue + '';
            const [input, setInput] = useState(init);

            const saveNewInitValue = () => setEditorColumns(state => {
                const temp = [...state];
                temp[index].initValue = +input;
                return temp;
            })

            return <th className={styles.initValues}>
                <div style={{ display: 'flex2', width: '100%' }}>
                    <input type="number" value={input + ''} onChange={({ target: { value } }) => setInput(value)} onKeyDown={event => {
                    ...
                    
                    {
                        ...
                        <div className={styles.tableControl}>
                            ...
                            <div className={styles.buttonsWrap}>
                                ...
                                <button onClick={saveTableView} disabled={newTableName.length < 5}>Сохранить схему отображения таблицы</button>
                            </div>
                            <div className={styles.addColumnBtn} onClick={() => addEditorColumn()}>Добавить колонку</div>    
                        </div>
                    }                             
                    ...
                    <div className={styles.selectedColumnBlock} ref={columnMenuRef} style={{ display: selectedColumn !== null ? 'flex' : 'none' }}>
                        ...
                        <div className={styles.buttonsBlock}>                            
                            ...
                            <button
                                ...
                                onClick={saveColumn}>Сохранить</button>
    ...
    return (
        <div className={styles.statsScreenWrapper}>
          ...
          <select value={selectedTableId} onChange={event => setSelectedTableId(+event.target.value)}>
                {
                isAdmin?
                <option value={0}>редактор таблиц</option>
                :<option value={0}>начальные колонки</option>
            }
                {
...

            {
                ...
                <MultiLinesChart chartSchema={chartSchema} records={statArrFiltered} costumLines={costumLinesArr} />
            }
