...
export default function FilterStat({ postItem, setTableSelect, clearTable }
    : {
        postItem: any,
    ...
    const StatDiv = ({ statId }: { statId: number | null }) => {
        ...
        if (statId) {
            return <div onClick={() => { setTableSelect(statId); setAdditionalStatsSelect(0) }}>{clearStatName(statNameById(statId))} 📈</div>
        } else {
    ...
    useEffect(() => { // ON CHANGE TARGET
        setAdditionalStatsSelect(0);
        if (currentTarget?.patterns?.length) { // FILL ADDITIONAL STATS LIST
    ...
    useEffect(() => { //ON ADDITIONAL STAT SELECT
        ...
        <div className={styles.mainFilterWrap}>
            ...
            <div className={styles.filterBlock}>
                {
                    ...
                    <div className={styles.filterItem}>
                        <select className={styles.depSelect} value={departmentSelect} onChange={event => setDepatmentSelect(+event.target.value)}>
                            ...
                            {
                                departmentsList.map(department => <option key={nanoid()} value={department.id}>{department.name}</option>)
                            }
                        </select>
                        {
                            !!departmentSelect &&
                            <div className={styles.close} onClick={() => setDepatmentSelect(0)}>❌</div>
                        }
                    </div>
                }
                {
                    ...
                    <div className={styles.filterItem}>
                        <select className={styles.secSelect} value={sectionSelect} onChange={event => setSectionSelect(+event.target.value)}>
                            ...
                            {
                                sectionsList.map(section => <option key={nanoid()} value={section.id}>{section.name}</option>)
                            }
                        </select>
                        {
                            !!sectionSelect &&
                            <div className={styles.close} onClick={() => { setSectionSelect(0) }}>❌</div>
                        }
                    </div>
                }
            </div>
            <div className={`${styles.targetWrap} ${styles[getTargetType()]}`}>
                ...
                <div className={styles.statStatus}>
                    {
                        currentTarget?.name == postItem?.name
                            ? <div className={styles.fill}><span>Заполняемая</span></div>
                ...
                <div className={styles.statsBlock}>
                    <div className={styles.mainStat}><span>Главная статистика</span><StatDiv statId={currentTarget.mainPattern} /></div>
                        ...
                        <span>Дополнительные статистики</span>
                        {
                            !!additionalStatsList.length
                                ...
                                <select value={additionalStatsSelect} onChange={event => setAdditionalStatsSelect(+event.target.value)}>
                                    ...
                                    {
                                        additionalStatsList.map(statId => <option key={nanoid()} value={statId}>{clearStatName(statNameById(statId))} 📉</option>)
                                    }
                                </select>
