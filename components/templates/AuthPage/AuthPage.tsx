...
export default function AuthPage() {
    ...
    const singInUpToggle = () => setSeingInUp(state => {
        setLoginEmail('');
        setLoginPassword('');
        setPost('');
        setName('');
            ...
            <AuthLayout>
                <div className={styles.wrapper}>
                        ...
                        singInUp
                            ? <div className={styles.singIn}>
                                <div className={styles.inputWrap}>
                                    <h3>email</h3>
                                    <input type="text" placeholder="email" value={loginEmail} onChange={event => setLoginEmail(event.target.value)} />
                                    <h3>пароль</h3>
                                    <input type="password" placeholder="пароль" value={loginPassword} onChange={event => setLoginPassword(event.target.value)} />
                                    <button onClick={() => singIn(loginEmail, loginPassword)} disabled={loading}>Войти</button>
                                </div>
                            </div>
                            : <div className={styles.singIn}>
                                <div className={styles.inputWrap}>
                                    ...
                                    <button onClick={singUpHandle} disabled={loading}>{singInUp?'Войти':'Зарегистрироваться'}</button>
                    ...
                    <div className={styles.authChoose} onClick={singInUpToggle} >
                        ...
                        {
                            singInUp
                                ? <span className={styles.choose}>Регистрации</span>
                                : <span className={styles.choose}>Авторизации</span>
                        }
                    </div>
                </div>
            </AuthLayout>
