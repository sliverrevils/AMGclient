...
export default function useStatistic() {
    ...
    const getAllUserStatsByChartId = async (id: number, setStatsArr: any) => {
        ...
        try {
            ...
            if (statsArr.data) {
                ...
                setStatsArr(statsArr.data.map((el) => ({ ...el, fields: JSON.parse(el.fields) })).sort((stat_1, stat_2) => new Date(+stat_1.dateStart).getTime() - new Date(+stat_2.dateStart).getTime()));
    ...
    const getPeriodByUserID = async (id: number = user.userId, pattern: number, dateStart: number, dateEnd: number, setStatsArr?: any): Promise<StatisticI[]> => {
        setLoading(true);
        try {
            const resArr: any = await axiosClient.post(`statistics/period/${id}`, {
                // if id = 0 : gets all users statistic
                pattern,
                dateStart,
                dateEnd,
            });
