...
export const useAccessRoutes=()=>{
    ...
    .filter(route=>route.access.some(role=>role===user.role))
    .map((el:any)=>({...el,clickFunc:()=>dispatch(setCurrentContentRedux(el.title)),active:el.title===currentAccessRoute?'active':''}));
